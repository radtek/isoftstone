...
CMeterItem2::CMeterItem2(QGraphicsItem *parent) :QGraphicsObject(parent)
{
    initVariables();
}

void CMeterItem2::initVariables()
{
    m_outerRadius=boundingRect().width()>boundingRect().height() ? boundingRect().height()/2: boundingRect().width()/2;
...
QRectF CMeterItem2::boundingRect() const
{
	return QRectF(0,0,200,200);
}	

void CMeterItem2::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
	painter->save();
...
void CMeterItem2::drawOuterCircle(QPainter *painter)
{
    painter->save();
...
void CMeterItem2::drawInnerCircle(QPainter *painter)
{
    painter->save();
...
void CMeterItem2::drawMark(QPainter *painter)
{
    painter->save();
...
void CMeterItem2::drawCoverBall(QPainter *painter)
{
    painter->save();
...
void CMeterItem2::drawTextRect(QPainter *painter)
{
    painter->save();
...
void CMeterItem2::drawCoverCircle(QPainter *painter)
{
    painter->save();
...
void CMeterItem2::drawColorPie(QPainter *painter)
{
    painter->save();
...
void CMeterItem2::drawIndicator(QPainter *painter)
{
    painter->save();
...
void CMeterItem2::resetVariables(QPainter *)
{
    m_outerRadius=boundingRect().width()>boundingRect().height() ? boundingRect().height()/2: boundingRect().width()/2;
    m_innerRadius=m_outerRadius/8*7;
    m_coverCircleRadius=m_outerRadius/8*4+10;
...
void CMeterItem2::setValue(qreal value)
{
    if(value>m_value)
    {
        m_bReverse=false;
