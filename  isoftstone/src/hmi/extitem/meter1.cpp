...
CMeterItem::CMeterItem(QGraphicsItem *parent) :QGraphicsObject(parent)
{
    m_value=0;
    m_currentValue=0;
    m_bReverse=false;
...
QRectF CMeterItem::boundingRect() const
{
	return QRectF(0,0,200,200);
}	

void CMeterItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
	painter->save();
...
void CMeterItem::resetVariables(QPainter *)
{
	m_outerRadius = boundingRect().width() > boundingRect().height() ? boundingRect().height()/2 : boundingRect().width()/2;
	m_innerRadius = 0.8*m_outerRadius;
	m_center = boundingRect().center();
...
void CMeterItem::drawOuterCircle(QPainter* painter)
{
    painter->save();
...
void CMeterItem::drawInnerCircle(QPainter* painter)
{
    painter->save();
...
void CMeterItem::drawColorPies(QPainter* painter)
{
    painter->save();
    painter->setPen(Qt::NoPen);
...
void CMeterItem::drawCoverLines(QPainter* painter)
{
    return ;
    painter->save();
...
void CMeterItem::drawCoverCircle(QPainter* painter)
{
    painter->save();
    painter->setPen(Qt::NoPen);
...
void CMeterItem::drawMarkAndText(QPainter* painter)
{
    painter->save();
    painter->translate(m_center);
...
void CMeterItem::drawGraph(QPainter* painter)
{
    qreal increment=(qreal)180/100;
    painter->save();
    painter->setPen(Qt::NoPen);
...
void CMeterItem::drawTextRect(QPainter* painter)
{
    painter->save();
...
void CMeterItem::setValue(qreal value)
{
    if(value>m_value)
    {
        m_bReverse=false;
