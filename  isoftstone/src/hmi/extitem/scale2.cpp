...
CScale2::CScale2(QGraphicsItem *parent) :QGraphicsObject(parent)
{
    mylinecolor=Qt::red;
    m_rectHeight=0;
    m_markLength=0;
...
void CScale2::setValue(qreal value)
{
    if(m_value<value)
    {
        m_bReverse=false;
...
QRectF CScale2::boundingRect() const
{
	return QRectF(0,0,400,80);
}	

void CScale2::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
...
void CScale2::drawTopBottomRects(QPainter *painter)
{
    painter->save();
    m_rectHeight=boundingRect().height()/6;

    QPointF topLeftPot1(0,0);
    QPointF bottomRightPot1(boundingRect().width(),m_rectHeight);
    ...
    QPointF topLeftPot2(0,boundingRect().height()-m_rectHeight);
    QPointF bottomRightPot2(boundingRect().width(),boundingRect().height());
    ...
    QPointF bottomRightPot3(boundingRect().width(),boundingRect().height()-m_rectHeight);
...
void CScale2::SetmyLineColor(QColor c)
{
    this->mylinecolor=c;
}

void CScale2::drawIndicator(QPainter *painter)
{
    painter->save();
...
void CScale2::drawMark(QPainter *painter)
{
    painter->save();
    ...
    for(int i=0;i<=100;i++)
    {
        if(i%10==0)
        {
            ...
            int fontWidth=painter->fontMetrics().width(strValue)/2;
