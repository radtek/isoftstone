...
CLCDNumber::CLCDNumber(QGraphicsItem *parent) :QGraphicsObject(parent)
{
    this->initVariables();
}

void CLCDNumber::initVariables()
{
    m_min=0;
    m_nNum=0;
...
void CLCDNumber::resize(qreal width,qreal height)
{
	m_width = width;
	m_height = height;
}

QRectF CLCDNumber::boundingRect() const
{
	return QRectF(0,0,m_width,m_width);
}	

void CLCDNumber::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
...
void CLCDNumber::drawVariables(QPainter *painter)
{
    m_center=boundingRect().center();
    m_min=qMin(boundingRect().width(),boundingRect().height());
    m_rectWidth=boundingRect().height()/8;
}

void CLCDNumber::drawBackground(QPainter *painter)
{
    painter->save();
...
void CLCDNumber::drawNumber(QPainter *painter)
{
    painter->save();
    painter->setPen(Qt::NoPen);
    ...
    QPointF top_topRightPot(boundingRect().width()-LCDNUMBER_SPACE-m_rectWidth,LCDNUMBER_SPACE);
    QPointF top_rightPot(boundingRect().width()-LCDNUMBER_SPACE-space,LCDNUMBER_SPACE+space);
    ...
    qreal polygon_height=(boundingRect().height()-3*LCDNUMBER_SPACE-3*m_rectWidth)/2;
...
void CLCDNumber::setValue(int num)
{
    m_nNum=num;
    update();
}
