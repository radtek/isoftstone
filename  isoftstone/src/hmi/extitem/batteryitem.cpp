...
CBatteryItem::CBatteryItem(QGraphicsItem *parent) :QGraphicsObject(parent)
{
    m_bReverse=false;
    m_value=100;
...
QRectF CBatteryItem::boundingRect() const
{
	return QRectF(0,0,120,40);
}	

void CBatteryItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
    drawFrame(painter);
    drawBattery(painter);
}
void CBatteryItem::drawFrame(QPainter *painter)
{
    painter->save();
    m_headWidth=boundingRect().width()/20;
    qreal batteryWidth=boundingRect().width()-m_headWidth;

    QPointF topLeftPot(0,0);
    QPointF bottomRightPot(batteryWidth,boundingRect().height());
    ...
    QPointF headRectTopLeft(batteryRect.topRight().x(),boundingRect().height()/4);
    QPointF headRectBottomRight(boundingRect().width(),boundingRect().height()-boundingRect().height()/4);
...
void CBatteryItem::drawBattery(QPainter *painter)
{
    painter->save();

    QLinearGradient batteryGradient(QPointF(0,0),QPointF(0,boundingRect().height()));
...
void CBatteryItem::setValue(qreal value)
{
