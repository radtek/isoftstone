#ifndef CLASS_ACLINE_AUTO_H
#define CLASS_ACLINE_AUTO_H
...
struct ACLine_Struct
{
	...
	ACLine_Struct& operator=(const ACLine_Struct& cp)
	{
		this->af = 		cp.af;
		this->app1 = 		cp.app1;
		this->app2 = 		cp.app2;
...
class CACLine_Auto : public ISerialize,public GSerialize,public CGraphicsItemBase 
{
public:
	...
	virtual void serialize(xml_node<char>* node) const
	{
		...
		attr = new xml_attribute<char>();
		attr->name( "link" );
		attr->value( m_Info.link.toLocal8Bit().data() );
		node->append_attribute(attr) ;
	...
	virtual void deserialize( xml_node<char>* node)
	{
		...
		attr = node->first_attribute("link");
		if(attr)
		{
			 m_AttrInfo["link"] = QString(attr->value());
			 m_Info.link = QObject::tr(attr->value());
		}
	...
	ACLine_Struct getInfo()
	{
		return m_Info;
	}
protected:

	ACLine_Struct m_Info ;
};
