#ifndef CLASS_LINE_AUTO_H
#define CLASS_LINE_AUTO_H
...
struct Line_Struct
{
	...
	Line_Struct& operator=(const Line_Struct& cp)
	{
		this->af = 		cp.af;
		this->d = 		cp.d;
		this->fc = 		cp.fc;
...
class CLine_Auto : public ISerialize,public GSerialize,public CGraphicsItemBase 
{
public:
	...
	virtual void serialize(xml_node<char>* node) const
	{
		...
		attr = new xml_attribute<char>();
		attr->name( "x1" );
		attr->value(QString::number(m_Info.x1).toLocal8Bit().data());
		node->append_attribute(attr) ;

		attr = new xml_attribute<char>();
		attr->name( "x2" );
		attr->value(QString::number(m_Info.x2).toLocal8Bit().data());
		node->append_attribute(attr) ;

		attr = new xml_attribute<char>();
		attr->name( "y1" );
		attr->value(QString::number(m_Info.y1).toLocal8Bit().data());
		node->append_attribute(attr) ;

		attr = new xml_attribute<char>();
		attr->name( "y2" );
		attr->value(QString::number(m_Info.y2).toLocal8Bit().data());
		node->append_attribute(attr) ;
	...
	virtual void deserialize( xml_node<char>* node)
	{
		...
		if(attr)
		{
			 m_AttrInfo["x1"] = QString(attr->value());
			 m_Info.x1 = QString(attr->value()).toInt();
		}

		attr = node->first_attribute("x2");
		if(attr)
		{
			 m_AttrInfo["x2"] = QString(attr->value());
			 m_Info.x2 = QString(attr->value()).toInt();
		}

		attr = node->first_attribute("y1");
		if(attr)
		{
			 m_AttrInfo["y1"] = QString(attr->value());
			 m_Info.y1 = QString(attr->value()).toInt();
		}

		attr = node->first_attribute("y2");
		if(attr)
		{
			 m_AttrInfo["y2"] = QString(attr->value());
			 m_Info.y2 = QString(attr->value()).toInt();
		}
