#ifndef CLASS_TRANSFORMER3_AUTO_H
#define CLASS_TRANSFORMER3_AUTO_H
...
struct Transformer3_Struct
{
	...
	Transformer3_Struct& operator=(const Transformer3_Struct& cp)
	{
		this->af = 		cp.af;
		this->app1 = 		cp.app1;
		this->app2 = 		cp.app2;
...
class CTransformer3_Auto : public ISerialize,public GSerialize,public CGraphicsItemBase 
{
public:
	...
	virtual void serialize(xml_node<char>* node) const
	{
		...
		attr = new xml_attribute<char>();
		attr->name( "app3" );
		attr->value(QString::number(m_Info.app3).toLocal8Bit().data());
		node->append_attribute(attr) ;
		...
		attr = new xml_attribute<char>();
		attr->name( "keyid3" );
		attr->value(QString::number(m_Info.keyid3).toLocal8Bit().data());
		node->append_attribute(attr) ;
	...
	virtual void deserialize( xml_node<char>* node)
	{
		...
		attr = node->first_attribute("app3");
		if(attr)
		{
			 m_AttrInfo["app3"] = QString(attr->value());
			 m_Info.app3 = QString(attr->value()).toULongLong();
		}
		...
		attr = node->first_attribute("keyid3");
		if(attr)
		{
			 m_AttrInfo["keyid3"] = QString(attr->value());
			 m_Info.keyid3 = QString(attr->value()).toULongLong();
		}
	...
	Transformer3_Struct getInfo()
	{
		return m_Info;
	}
protected:

	Transformer3_Struct m_Info ;
};
