#ifndef CLASS_STATUS_AUTO_H
#define CLASS_STATUS_AUTO_H
...
struct Status_Struct
{
	...
	Status_Struct& operator=(const Status_Struct& cp)
	{
		this->af = 		cp.af;
		this->app = 		cp.app;
		...
		this->p_MultiSelect = 		cp.p_MultiSelect;
...
class CStatus_Auto : public ISerialize,public GSerialize,public CGraphicsItemBase 
{
public:
	...
	virtual void serialize(xml_node<char>* node) const
	{
		...
		attr = new xml_attribute<char>();
		attr->name( "p_MultiSelect" );
		attr->value( m_Info.p_MultiSelect.toLocal8Bit().data() );
		node->append_attribute(attr) ;
	...
	virtual void deserialize( xml_node<char>* node)
	{
		...
		attr = node->first_attribute("p_MultiSelect");
		if(attr)
		{
			 m_AttrInfo["p_MultiSelect"] = QString(attr->value());
			 m_Info.p_MultiSelect = QObject::tr(attr->value());
		}
	...
	Status_Struct getInfo()
	{
		return m_Info;
	}
protected:

	Status_Struct m_Info ;
};
