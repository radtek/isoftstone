...
void MibII::GetNICInfo(tSTRUCTNICINFO *pNICInfo)
{
	...
	for (i = 0; i < m_ifCount; i++)
	{
		...
		// LGCH 安装 vnn 软件会创建隐藏网卡，造成对比失败，此时应跳过以下代码
		if (j < m_ifCount)
		{
...
BOOL CMibWrapper::CheckRegist (LPCSTR szCode)
{
	std::string reg = getRegist();
	return strcmp (reg.c_str(), szCode) == 0 ? TRUE : FALSE;
}

std::string CMibWrapper::getRegist()
{
	std::string ret = "";
	MibII m;
	int rv = m.Init();
	if (rv < 0)
		return ret;
	//	SBLSaveRunInfo ("开始校验注册码!");
	int NICCount = m.GetNICCount(FALSE, FALSE);
	if (NICCount > 0)
	{
		//		SBLSaveRunInfo ("读取本机MAC地址成功!");
		tSTRUCTNICINFO* pNICInfo = new tSTRUCTNICINFO[NICCount];
		memset (pNICInfo, 0, sizeof(tSTRUCTNICINFO)*NICCount);
		m.GetNICInfo(pNICInfo);
		char szMACCode[256];
		char szRegistCode[256];
		for(int i=0; i<NICCount; i++)	// first address is
			// 00000000, skip it
		{										// enumerate MACs
			memset (szMACCode, 0, 256);
			char *p = szMACCode;
			for (UINT m=0; m<pNICInfo[i].MACLength; m++)
			{
				sprintf(p, "%02X.", pNICInfo[i].MAC[m]);
				p += 3;
			}
			szMACCode[strlen(szMACCode)-1] = '\0';
			if (SBDesEncrypt (szMACCode) == FALSE)
			{
				//SBLSaveRunInfo ("MAC地址转换失败!");
				return ret;
			}
			memset (szRegistCode, 0, 256);
			p = szRegistCode;
			for (UINT r=0; r<strlen(szMACCode); r++)
			{
				sprintf(p, "%0.3d-", (BYTE)szMACCode[r]);
				p += 4;
			}
			szRegistCode[strlen(szRegistCode)-1] = '\0';
